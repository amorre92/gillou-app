...
const ViventLesBleus = () => {
    // measure 1
    // notes
    let note1_m1 = new Vex.Flow.StaveNote({ keys: ["a/4"], duration: "8" }).setStemDirection(-1)
    let note2_m1 = new Vex.Flow.StaveNote({ keys: ["c/5"], duration: "8" }).setStemDirection(-1)
    let note3_m1 = new Vex.Flow.StaveNote({ keys: ["b/4"], duration: "8" }).setStemDirection(-1)
