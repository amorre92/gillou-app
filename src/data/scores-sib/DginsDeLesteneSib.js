...
const DginsDeLesteneSib = () => {
  ...
  let note2_m1 = new StaveNote({ keys: ["c/5"], duration: "16" });
  let note3_m1 = new StaveNote({ keys: ["d/5"], duration: "16" });
  let note4_m1 = new StaveNote({ keys: ["c/5"], duration: "8" });
  let note5_m1 = new StaveNote({
    keys: ["b/4"],
    duration: "8",
  }).addModifier(new Accidental("n"));
  let note6_m1 = new StaveNote({ keys: ["c/5"], duration: "8" });
  // beams
